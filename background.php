...
function getStats(){
    ...
    exec("goaccess -f /var/log/apache2/access.log --log-format=COMBINED -a -o /tmp/report.html", $output, $rval);
...
function addModules($moddirs) {
    ...
    foreach ($moddirs as $moddir) {
        ...
        //$cmd = "rsync -Pav$zip  --no-links --no-o --no-g --del rsync://$host/rachelmods/$moddir $relmodpath/";
