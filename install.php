...
function populateLocalModuleList() {
    ...
    $.ajax({
        ...
        success: function(results) {
            ...
            var arrayLength = results.length;
            for (var i = 0; i < arrayLength; i++) {
                addToLocal(results[i].moddir, results[i].ksize);
            }
        },
...
function kilobytesToGigabytes(kb) {
    return (kb / 1024 / 1024).toFixed(2); // returns GB as a string with 2 decimals
}

function addToLocal (moddir, ksize = 0) {
    ...
    if (!$(sel(moddir)).length
        ...
        $("#modlist").append(
            ...
            "')\">delete</button> <small>" + kilobytesToGigabytes(ksize) + "GB</small> </li> \n"
        );
    }
    delete current_task_moddirs[ moddir ];
}
